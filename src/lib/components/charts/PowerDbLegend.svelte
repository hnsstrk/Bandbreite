...
<g transform="translate({chartWidth / 2 - 300}, {chartHeight + 65})">
  ...
  <circle
    ...
    cy="0"
    r="6"
    fill={categoryColors.communication}
  ...
  <!-- Satellite legend -->
  <polygon
    points="200,-6 206,4 194,4"
    fill={categoryColors.satellite}
    stroke="var(--color-line-strong)"
    stroke-width="1.5"
  />
  ...
  <!-- IoT legend -->
  <polygon
    points="285,0 293,-6 301,0 293,6"
    fill={categoryColors.iot}
    stroke="var(--color-line-strong)"
    stroke-width="1.5"
  />
  ...
  <rect
    ...
    height="12"
    rx="1"
    fill={bandMode === 'ieee' ? 'var(--color-series-4)' : 'var(--color-series-2)'}
    opacity="0.5"
  />
  <text x="498" y="4" style="fill: var(--color-text-tertiary)" font-size="11"
    >{bandMode === 'ieee' ? 'IEEE' : 'NATO'}-Bänder</text
  >
</g>
