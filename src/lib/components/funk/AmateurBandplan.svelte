<script lang="ts">
	...
	const selectedBand = $derived(
		selected ? (bands.find((band) => band.id === selected!.bandId) ?? null) : null
	);
	const selectedSegment = $derived(
		selectedBand && selected ? (selectedBand.segments[selected.index] ?? null) : null
	);
	...
	function select(bandId: string, index: number) {
		selected =
			selected && selected.bandId === bandId && selected.index === index
				? null
				: { bandId, index };
	}
</script>
...
<style>
	...
	.seg--cw { background-color: var(--color-series-1); }
	.seg--digital-schmal { background-color: var(--color-series-2); }
	.seg--digital-breit { background-color: var(--color-series-5); }
	.seg--bake { background-color: var(--color-series-7); }
	.seg--ssb { background-color: var(--color-series-3); }
	.seg--allmode { background-color: var(--color-series-4); }
	.seg--fm { background-color: var(--color-series-6); }
	.seg--satellit { background-color: var(--color-series-9); }
	.seg--atv { background-color: var(--color-series-8); }
