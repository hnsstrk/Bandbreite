<script lang="ts">
  ...
  const suffixOptions = SUFFIXES.map((value) => ({
    value,
    label:
      value.endsWith('00') || value.endsWith('25') || value.endsWith('50') || value.endsWith('75')
        ? `…${value} (25 kHz)`
        : `…${value} (8,33 kHz)`
  }));
  ...
  const offsetHz = $derived(channel ? channel.frequencyHz - (Number(megahertz) * 1e6 + Number(suffix) * 1000) : 0);
</script>
...
>
  ...
  {#snippet results()}
    <ResultCard
      ...
      value={designator}
      secondary={channel?.spacingHz === VHF_SPACING_25_HZ ? 'klassischer 25-kHz-Kanal' : 'Kanal im 8,33-kHz-Raster'}
      emphasis="hero"
    />
    <ResultCard
...
<Callout tone="warning" title="Bezeichnung ist nicht Frequenz">
  Die Freigabe „contact Radar one one eight decimal zero one zero" meint den Kanal 118.010 — der Sender arbeitet dabei
  auf 118,008 333 MHz. Bezeichnungen mit den Endungen 20, 45, 70 und 95 gibt es nicht; sie wären im Sprechfunk zu leicht
  mit den vergebenen zu verwechseln.
</Callout>
