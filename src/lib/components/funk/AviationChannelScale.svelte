...
<svg viewBox="0 0 {W} {H}" aria-hidden="true">
  ...
  {#each slots as slot (slot.index)}
    ...
    />
    <text x={xOf(baseHz + slot.startHz) + (X1 - X0) / SLOTS / 2} y="32" text-anchor="middle" class="chart-axis-text">
  ...
  {#each marks as mark (mark.designator)}
    ...
    />
    {#if !mark.wide}
      <text x={mark.x} y={AXIS_Y + 18} text-anchor="middle" class="tick" class:tick--active={mark.active}>
  ...
  <text x={(X0 + X1) / 2} y={H - 8} text-anchor="middle" class="chart-axis-text">
    Kanalbezeichnung (letzte drei Stellen) · drei Kanäle im Abstand {formatFixed(VHF_SPACING_833_HZ / 1000, 2)} kHz je Rasterplatz
  </text>
</svg>
