<script lang="ts">
  ...
  const xOf = (year: number) => X0 + ((year - YEAR_MIN) / (YEAR_MAX - YEAR_MIN)) * (X1 - X0);
...
>
  <svg viewBox="0 0 {W} {H}">
    ...
    <rect x={xOf(switchYear)} y={AXIS_Y - 10} width={X1 - xOf(switchYear)} height="20" fill="var(--color-brand-soft)" />
    <text x={(X0 + xOf(switchYear)) / 2} y={AXIS_Y + 46} text-anchor="middle" class="era"> analog · 4 m und 2 m </text>
    ...
    {#each BOS_MILESTONES as milestone, index (milestone.labelDE)}
      ...
      >
        ...
        <text {x} {y} text-anchor="middle" class="year" class:year--active={index === selected}>
          {milestone.labelDE}
        </text>
      </g>
    {/each}
