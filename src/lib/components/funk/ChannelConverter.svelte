    ...
    {#snippet panel(id)}
      {#if id === 'dab'}
        <div class="pane">
          ...
          {/if}
          <p class="note">
            Ein DAB-Ensemble belegt immer einen ganzen Block und trägt darin mehrere Programme im Zeitmultiplex. Alle
            Sender eines Gleichwellennetzes senden denselben Block mit demselben Inhalt und synchronisiert — der
            Empfänger wertet die Echos als Gewinn. Deutschland nutzt die Blöcke 5A bis 12D zwischen 174 und 230 MHz; die
            Zwischenblöcke 10N, 11N und 12N sowie der Kanal 13 mit 13A bis 13F (230 bis 240 MHz) sind Teil der
            Kanaltabelle, hier aber nicht belegt.
          </p>
        </div>
      {:else if id === 'dvbt2'}
        <div class="pane">
          ...
          <div class="results">
            <ResultCard
              ...
              secondary={channel ? formatFrequencyRange(channel.minHz, channel.maxHz) : undefined}
          ...
          <p class="note">
            Die Kanalmitte folgt der Regel 306 MHz + 8 MHz · Kanalnummer. Seit der Umwidmung des 700-MHz-Bereichs endet
            der Fernsehbereich bei 694 MHz, also mit Kanal 48.
          </p>
        </div>
      {:else}
        <div class="pane">
          ...
          <p class="note">
            Der Frequenzhub beträgt höchstens 75 kHz, das Modulationssignal reicht mit dem Stereo-Differenzsignal bis 53
            kHz und mit RDS bis 57 kHz. Nach der Carson-Regel ergibt sich daraus eine belegte Bandbreite von rund 180
            kHz; benachbarte Sender am selben Ort halten deshalb mehrere Rasterplätze Abstand.
          </p>
        </div>
      {/if}
    {/snippet}
