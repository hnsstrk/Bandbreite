<script lang="ts">
  ...
  const BANDWIDTH_UNITS = FREQUENCY_UNITS.filter((unit) => ['kHz', 'MHz', 'GHz'].includes(unit.id));
  ...
  const efficiency = $derived(EFFICIENCY_PRESETS.find((preset) => preset.id === efficiencyId) ?? EFFICIENCY_PRESETS[1]);
...
<Panel title="Bandbreite → Datenrate" subtitle="Brutto-Abschätzung ohne Protokoll- und Signalisierungsanteile">
  <div class="grid">
    ...
    />
    <Select label="Spektrale Effizienz" bind:value={efficiencyId} options={efficiencyChoices} hint={efficiency.hint} />
    <Select
      label="Räumliche Ströme"
      bind:value={() => String(layers), (next: string) => (layers = Number(next))}
  ...
  <div class="results">
    ...
    />
    <ResultCard label="Je Strom" value={formatDataRate(singleLayerRate, 1)} hint="Ohne MIMO-Gewinn" />
  </div>
