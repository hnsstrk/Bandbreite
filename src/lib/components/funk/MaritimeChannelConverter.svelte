<script lang="ts">
	...
	const channelOptions = $derived(
		filtered.map((entry) => ({ value: entry.channel, label: `Kanal ${entry.channel}` }))
	);
	...
	function handleUsageChange(value: string) {
		...
		const stillVisible = maritimeChannelsByUsage(usage).some(
	...
	const hitLabel = $derived(
		hits.length === 0
			...
			: hits
					.map(
						(hit) =>
							`Kanal ${hit.channel.channel} (${hit.direction === 'schiff' ? 'Schiff' : 'Küste'})`
					)
					.join(' · ')
	);
</script>
...
<Card
	title="Seefunk-Kanal und Frequenz umrechnen"
	subtitle="UKW-Seefunkband nach VO Funk Appendix 18"
>
	{#snippet actions()}
