<script lang="ts">
	...
	const visibilityLabel = $derived(
		orbit.periodS > 12 * HOUR_S
			? 'dauerhaft'
			: `${formatNumber(orbit.visibilityDurationS / MINUTE_S, 1)} min`
	);

	/** Kurzname der Bahnklasse für die Beschriftung der Bühne. */
	const orbitLabel = $derived(
		altitudeM >= 30_000_000
			? 'geostationär'
			: altitudeM >= 2_000_000
				? 'mittlere Bahn'
				: 'niedrige Bahn'
...
>
	...
	{#snippet results()}
		...
		<ResultCard
			...
			value={`${formatNumber(orbit.velocityMs / 1000, 2)} km/s`}
		/>
		<ResultCard
		...
		/>
		<ResultCard
			label="Doppler (obere Schranke)"
			value={formatFrequency(dopplerHz, 1)}
			hint="Δf = f · v / c"
		/>
	{/snippet}
	...
	{#snippet dataTable()}
		<table>
			...
			<tbody>
				...
				<tr><th scope="row">Signallaufzeit hin und zurück</th><td>{formatNumber(orbit.roundTripDelayS * MS_PER_S, 1)} ms</td></tr>
