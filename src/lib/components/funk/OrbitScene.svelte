<script lang="ts">
	...
	const orbitPx = $derived(
		EARTH_PX + (MAX_PX - EARTH_PX) * Math.min(Math.log(ratio) / Math.log(MAX_RATIO), 1)
	);
...
<svg viewBox="0 0 {W} {H}" aria-hidden="true">
	...
	{#each edges as edge, i (i)}
		<line x1={satX} y1={satY} x2={edge.x} y2={edge.y} stroke="var(--color-series-3-solid)" stroke-width="1" opacity="0.7" />
	{/each}
