<script lang="ts">
	...
	const rows = $derived(
		buildServiceRows(filterServices(groupValue as RadioServiceGroup | 'alle'))
	);
	const markerPercent = $derived(
		logPositionPercent(markerHz, SERVICE_SCALE_MIN_HZ, SERVICE_SCALE_MAX_HZ)
	);
					...
					>
						...
						<span class="sr-only">
							...
							{row.service.allocations
								.map(
									(a) =>
										`${a.label} ${formatFrequencyRange(a.minHz, a.maxHz)} ${ALLOCATION_STATUS_LABELS[a.status]}`
								)
								.join('; ')}
						</span>
					</button>
...
<style>
	...
	.seg--sekundaer { opacity: 0.45; }
	.seg--gemischt { opacity: 0.7; }
	.seg--rundfunk { background-color: var(--color-series-1); }
	.seg--mobil { background-color: var(--color-series-2); }
	.seg--fest { background-color: var(--color-series-3); }
	.seg--navigation { background-color: var(--color-series-4); }
	.seg--ortung { background-color: var(--color-series-5); }
	.seg--satellit { background-color: var(--color-series-6); }
	.seg--wissenschaft { background-color: var(--color-series-7); }
	.seg--sicherheit { background-color: var(--color-series-9); }
	.seg--amateur { background-color: var(--color-series-8); }
