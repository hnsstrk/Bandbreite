<script lang="ts">
  ...
  let { items, title = 'Auf dieser Seite', sticky = true, compactBelow = 1280, class: klass = '' }: Props = $props();
  ...
  <ol class="ui-toc__list">
    {#each items as item (item.id)}
      <li class="ui-toc__entry ui-toc__entry--l{item.level}">
        <a class="ui-toc__link" href="#{item.id}" aria-current={activeId === item.id ? 'location' : undefined}>
          {item.label}
        </a>
      </li>
    {/each}
...
{:else}
  <nav class="ui-toc {sticky ? 'ui-toc--sticky' : ''} {klass}" aria-labelledby={headingId}>
