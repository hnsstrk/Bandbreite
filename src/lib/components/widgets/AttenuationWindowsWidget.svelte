...
>
	<svg viewBox="0 0 {W} {H}" bind:this={svgElement} onpointermove={handlePointer} onpointerdown={handlePointer} aria-hidden="true">
		...
		<text x="14" y={M.top + PLOT_H / 2} text-anchor="middle" class="chart-axis-label" transform="rotate(-90, 14, {M.top + PLOT_H / 2})">dB/km</text>
		{#each WINDOW_MARKERS as marker (marker.id)}
			<line x1={xOf(marker.frequencyGHz)} y1={M.top} x2={xOf(marker.frequencyGHz)} y2={M.top + PLOT_H} stroke={marker.kind === 'peak' ? 'var(--color-series-6)' : 'var(--color-series-2)'} stroke-width="1" stroke-dasharray={marker.kind === 'peak' ? '3,3' : '6,3'} opacity="0.6" />
			<text x={xOf(marker.frequencyGHz)} y={marker.kind === 'peak' ? M.top - 8 : M.top + 12} text-anchor="middle" class="chart-legend-text" fill={marker.kind === 'peak' ? 'var(--color-danger-ink)' : 'var(--color-success-ink)'}>{marker.label}</text>
		{/each}
		...
		{#if probe}
			<line x1={xOf(probe.frequencyGHz)} y1={M.top} x2={xOf(probe.frequencyGHz)} y2={M.top + PLOT_H} class="chart-marker-crosshair" />
			<circle cx={xOf(probe.frequencyGHz)} cy={yOf(probe.total)} r="6" class="chart-marker-primary" stroke="var(--color-ink)" stroke-width="1.5" />
		{/if}
		<g transform="translate({M.left + 10}, {M.top + PLOT_H - 44})">
			<line x1="0" y1="0" x2="18" y2="0" stroke="var(--color-series-3)" stroke-width="2.5" /><text x="24" y="4" class="chart-legend-text">gesamt</text>
			<line x1="0" y1="14" x2="18" y2="14" stroke="var(--color-series-1)" stroke-width="1.5" /><text x="24" y="18" class="chart-legend-text">Sauerstoff</text>
			<line x1="0" y1="28" x2="18" y2="28" stroke="var(--color-series-2)" stroke-width="1.5" /><text x="24" y="32" class="chart-legend-text">Wasserdampf</text>
		</g>
	</svg>
	...
	{#snippet dataTable()}
		<table>
			...
			<tbody>
				{#each WINDOW_MARKERS as marker (marker.id)}
					<tr><td>{marker.label}</td><td>{marker.kind === 'peak' ? 'Resonanz' : 'Fenster'}</td><td>{formatAttenuation(findNearestPoint(curve, marker.frequencyGHz)?.total)}</td></tr>
				{/each}
			</tbody>
		</table>
	{/snippet}
	...
	{#snippet controls()}
		<Slider label="Abgelesene Frequenz" bind:value={probeGHz} min={WINDOW_CURVE_MIN_GHZ} max={WINDOW_CURVE_MAX_GHZ} scale="log" format={formatGHz} unitSymbol="GHz" hint="Auch per Zeiger direkt im Diagramm" />
		<Slider label="Wasserdampfdichte" bind:value={waterVaporDensity} min={WATER_VAPOR_LIMITS.min} max={WATER_VAPOR_LIMITS.max} step={0.5} format={(v) => `${formatNumber(v, 1)} g/m³`} unitSymbol="g/m³" ticks={[{ at: 0, label: 'trocken' }, { at: 7.5, label: 'Standard' }, { at: 30, label: 'tropisch' }]} />
	{/snippet}

	{#snippet results()}
		{#if probe}
			<ResultCard label="γ bei {formatGHz(probe.frequencyGHz)}" value={formatAttenuation(probe.total)} emphasis="hero" copyable={false} />
