<script lang="ts">
  ...
  const vMaxMs = $derived(Math.max(first.blindSpeedsMs.at(-1) ?? 1, second.blindSpeedsMs.at(-1) ?? 1) * 1.05);
  ...
  const targetVisible = $derived(Math.max(targetResponse1, targetResponse2) >= SUPPRESSION_THRESHOLD);
</script>
...
>
  <svg viewBox="0 0 {W} {H}" aria-hidden="true">
    ...
    <path
      d={curve(second.firstBlindSpeedMs)}
      fill="none"
    ...
    {#if stagger.commonBlindSpeedMs > 0 && stagger.commonBlindSpeedMs <= vMaxMs}
      <line
        x1={x(stagger.commonBlindSpeedMs)}
        y1={y(0)}
        x2={x(stagger.commonBlindSpeedMs)}
        y2={y(1)}
        stroke="var(--color-series-3)"
        stroke-width="2"
      />
      <text x={x(stagger.commonBlindSpeedMs)} y={PAD.top - 6} text-anchor="middle" class="chart-legend-text"
        >gemeinsame Lücke</text
      >
    {/if}

    <circle
      cx={x(targetV)}
      cy={y(Math.max(targetResponse1, targetResponse2))}
      r="6"
      fill={targetVisible ? 'var(--color-series-3)' : 'var(--color-ink-subtle)'}
    />
    <text
      x={x(targetV)}
      y={y(Math.max(targetResponse1, targetResponse2)) - 12}
      text-anchor="middle"
  ...
  {#snippet dataTable()}
    <table>
      ...
      <tbody>
        <tr
          ><th>Sendefrequenz</th><td
            >{formatFrequency(frequencyHz, 2)} (λ = {formatNumber(first.wavelengthM * 100, 1)} cm)</td
          ></tr
        >
        ...
        <tr
          ><th>Weitere Blindgeschwindigkeiten</th><td
            >{first.blindSpeedsMs.map((v) => `${formatNumber(v, 1)} m/s`).join(', ')}</td
          ></tr
        >
  ...
  {#snippet results()}
    ...
    <Callout tone="tip" title="Warum staffeln?">
      Eine einzelne PRF lässt regelmäßig wiederkehrende Lücken im Geschwindigkeitsbereich. Wechselt das Radar von Impuls
      zu Impuls zwischen zwei Pulswiederholfrequenzen, fallen die Lücken nicht mehr zusammen — erst bei
      {stagger.commonBlindSpeedMs > 0
        ? `${formatNumber(stagger.commonBlindSpeedMs, 0)} m/s`
