<script lang="ts">
	...
	const xOf = (levelDbm: number) =>
		AXIS_X0 + ((Math.max(LEVEL_MIN, Math.min(LEVEL_MAX, levelDbm)) - LEVEL_MIN) / (LEVEL_MAX - LEVEL_MIN)) * (AXIS_X1 - AXIS_X0);
...
>
	<svg viewBox="0 0 {W} {H}" aria-hidden="true">
		...
		{#each chain as point, i (point.stage.id)}
			...
			<rect x={from} y={y} width={Math.max(2, to - from)} height={BAR_H} rx="3" fill={point.stage.db >= 0 ? 'var(--color-series-2)' : 'var(--color-series-6)'} opacity="0.85" />
			<text x={AXIS_X0 + 4} y={y + 15} class="chart-axis-text">{point.stage.label} {point.stage.db > 0 ? '+' : ''}{formatNumber(point.stage.db, 0)} dB</text>
			<text x={AXIS_X1} y={y + 15} text-anchor="end" class="chart-axis-text" font-weight="600">→ {formatNumber(point.levelDbm, 1)} dBm</text>
		{/each}
		<line x1={xOf(TYPICAL_RX_SENSITIVITY_DBM)} y1="8" x2={xOf(TYPICAL_RX_SENSITIVITY_DBM)} y2={H - 26} stroke="var(--color-series-6)" stroke-width="1" stroke-dasharray="3,3" />
		<text x={xOf(TYPICAL_RX_SENSITIVITY_DBM) + 4} y="18" class="chart-legend-text" fill="var(--color-danger-ink)">Empfindlichkeit {TYPICAL_RX_SENSITIVITY_DBM} dBm</text>
		...
		<table>
			...
			<tbody>
				{#each chain as point (point.stage.id)}
					<tr><td>{point.stage.label}</td><td>{formatNumber(point.stage.db, 0)} dB</td><td>{formatNumber(point.levelDbm, 1)} dBm</td></tr>
				{/each}
			</tbody>
		</table>
	{/snippet}
	...
	{#snippet controls()}
		<div class="db-grid">
			<div class="db-column">
				<Slider label="Verhältnis in dB" bind:value={db} min={DECIBEL_LIMITS.min} max={DECIBEL_LIMITS.max} step={0.5} format={(v) => `${v > 0 ? '+' : ''}${formatNumber(v, 1)} dB`} unitSymbol="dB" ticks={[{ at: -40, label: '−40' }, { at: 0, label: '0' }, { at: 40, label: '+40' }]} />
				<div class="db-chips" role="group" aria-label="Merkregeln">
					{#each DECIBEL_RULES as rule (rule.db)}
						<Button size="sm" variant={db === rule.db ? 'primary' : 'secondary'} pressed={db === rule.db} onclick={() => applyRule(rule.db)}>{rule.label}</Button>
					{/each}
				</div>
			</div>
			<div class="db-column">
				...
				{#each stages as stage (stage.id)}
					<Slider label={stage.label} bind:value={stage.db} min={stage.min} max={stage.max} step={1} format={(v) => `${v > 0 ? '+' : ''}${formatNumber(v, 0)} dB`} unitSymbol="dB" />
				{/each}
	...
	{#snippet results()}
		<ResultCard label="Leistungsfaktor 10^(dB/10)" value={formatNumberAuto(powerRatio)} secondary="{db > 0 ? '+' : ''}{formatNumber(db, 1)} dB" emphasis="hero" copyable={false} />
		<ResultCard label="Spannungsfaktor 10^(dB/20)" value={formatNumberAuto(voltageRatio)} copyable={false} />
		<ResultCard label="Empfangspegel der Kette" value={formatNumber(total, 1)} unit="dBm" tone={total >= TYPICAL_RX_SENSITIVITY_DBM ? 'success' : 'warning'} secondary={total >= TYPICAL_RX_SENSITIVITY_DBM ? `über typischer Empfindlichkeit ${TYPICAL_RX_SENSITIVITY_DBM} dBm` : `unter ${TYPICAL_RX_SENSITIVITY_DBM} dBm – Reserve fehlt`} copyable={false} />
	{/snippet}
</WidgetFrame>
