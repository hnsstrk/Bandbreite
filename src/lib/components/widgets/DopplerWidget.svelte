...
>
	<svg viewBox="0 0 {W} {H}" aria-hidden="true">
		...
		<g transform="translate({RADAR_X}, {BEAM_Y})">
			...
			<text x="0" y="60" text-anchor="middle" class="chart-axis-text">Radar · {formatFrequency(carrier.frequencyHz, 3)}</text>
		</g>
		...
		<g transform="translate({SPECTRUM_X0}, 178)">
			...
			<line x1={SPECTRUM_W / 2 + spectrumShift} y1="0" x2={SPECTRUM_W / 2 + spectrumShift} y2="-28" stroke="var(--color-series-2)" stroke-width="2" />
	...
	{#snippet dataTable()}
		<table>
			...
			<tbody>
				...
				<tr><th>Radialgeschwindigkeit</th><td>{formatNumber(velocityMs, 0)} m/s ({formatNumber(msToKmh(velocityMs), 0)} km/h)</td></tr>
	...
	{#snippet controls()}
		...
		<Slider
			...
			unitSymbol="m/s"
			ticks={[{ at: -300, label: '−300' }, { at: 0, label: '0' }, { at: 300, label: '+300' }]}
		/>
	{/snippet}
	...
	{#snippet results()}
		...
		<Callout tone="tip" title="Vorzeichen">
			Nähert sich das Ziel (v &gt; 0), trifft pro Sekunde mehr Wellenfronten ein – die Empfangsfrequenz steigt, f_d ist positiv. Entfernt es sich, sinkt sie. Der Faktor 2 entsteht, weil die Welle hin und zurück läuft.
		</Callout>
	{/snippet}
</WidgetFrame>
