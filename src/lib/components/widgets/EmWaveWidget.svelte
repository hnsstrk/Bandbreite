<script lang="ts">
	...
	function path(vertical: (s: (typeof samples)[number]) => number, depth: (s: (typeof samples)[number]) => number): string {
		return samples
...
>
	<svg viewBox="0 0 {W} {H}" aria-hidden="true">
		...
		{#each arrows as s, i (i)}
			...
			/>
		{/each}
		<path d={path((s) => s.hy, (s) => s.hz)} fill="none" stroke="var(--color-chart-blue)" stroke-width="2.5" />
		...
		{#each arrows as s, i (i)}
			...
			/>
		{/each}
		<path d={path((s) => s.ey, (s) => s.ez)} fill="none" stroke="var(--color-chart-red)" stroke-width="2.5" />
	...
	{#snippet results()}
		...
		<ResultCard
			...
			copyable={false}
		/>
		<ResultCard label="Fernfeld ab 2·D²/λ" value={formatDistance(farFieldM, 2)} hint="Nahfeld bis λ/2π = {formatDistance(nearFieldM, 3)}" copyable={false} />
	{/snippet}
</WidgetFrame>
