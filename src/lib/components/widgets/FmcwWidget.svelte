...
<WidgetFrame
	...
	footnote="Der Zeitversatz der Empfangsrampe ist zur Sichtbarkeit übertrieben: real beträgt er hier {formatNumber(result.delayFraction * 100, 2)} % der Rampendauer. Trägermitte {formatFrequency(FMCW_CENTER_FREQUENCY_HZ, 0)} (Automotive-Band)."
>
	...
	{#snippet results()}
		...
		<Callout tone={result.delayCritical ? 'warning' : 'tip'} title="Was die Regler bewirken">
			...
			trennen Ziele bis auf {formatWavelength(calculateBandwidthRangeResolution(FMCW_LIMITS.bandwidthHz.max), 1)}.
			Eine kürzere Rampe erhöht die Beat-Frequenz und verlangt einen schnelleren Analog-Digital-Umsetzer.
			{#if result.delayCritical}
				Hier belegt die Laufzeit bereits über {formatNumber(result.delayFraction * 100, 0)} % der Rampe — der
				gemeinsam nutzbare Teil beider Rampen wird knapp.
			{/if}
		</Callout>
	{/snippet}
</WidgetFrame>
