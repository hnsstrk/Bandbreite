<script lang="ts">
	...
	const scene = $derived(computeFresnelScene(distanceM, frequencyHz, obstaclePosition, obstacleHeightM, antennaHeightM));
	...
	const ellipsePath = $derived.by(() => {
		...
		const lower = [...scene.ellipse].reverse().map((p) => `${xOf(p.xM).toFixed(1)},${(losY + p.rM * pxPerM).toFixed(1)}`);
	...
	const clearancePath = $derived.by(() => {
		...
		const lower = [...scene.ellipse].reverse().map((p) => `${xOf(p.xM).toFixed(1)},${(losY + p.rM * f * pxPerM).toFixed(1)}`);
		return `M${upper.join(' L')} L${lower.join(' L')} Z`;
	});
	...
	const statusColor = $derived(
		status === 'frei' ? 'var(--color-success)' : status === 'eingeschraenkt' ? 'var(--color-warning)' : 'var(--color-danger)'
	);
	const statusLabel = $derived(
		status === 'frei' ? 'frei (≥ 60 %)' : status === 'eingeschraenkt' ? 'eingeschränkt (< 60 %)' : 'Sichtlinie blockiert'
	);
...
>
	<svg viewBox="0 0 {W} {H}" aria-hidden="true">
		...
		<path d={ellipsePath} fill="var(--color-series-1)" opacity="0.18" stroke="var(--color-series-1)" stroke-width="1.5" />
		...
		{#each [X0, X1] as x, i (i)}
			...
			<path d="M{x - 14},{losY - 10} L{x},{losY + 2} L{x + 14},{losY - 10}" fill="none" stroke="var(--color-series-1-solid)" stroke-width="2.5" />
		{/each}
		<text x={X0} y={GROUND_Y + 18} text-anchor="middle" class="chart-axis-text">Sender ({formatDistance(antennaHeightM, 0)})</text>
		<text x={X1} y={GROUND_Y + 18} text-anchor="middle" class="chart-axis-text">Empfänger ({formatDistance(antennaHeightM, 0)})</text>
		<!-- Hindernis -->
		<polygon points="{obstacleX - 26},{GROUND_Y} {obstacleX},{obstacleTopY} {obstacleX + 26},{GROUND_Y}" fill={statusColor} opacity="0.85" />
		...
		<text x={X1} y={H - 6} text-anchor="end" class="chart-axis-text">Strecke {formatDistance(distanceM, 1)} · {formatFrequency(frequencyHz, 2)}</text>
	</svg>
	...
	{#snippet dataTable()}
		<table>
			...
			<tbody>
				...
				<tr><th>Freier Anteil</th><td>{formatPercentage(scene.clearance.clearanceFraction * 100, 0)} – {statusLabel}</td></tr>
	...
	{#snippet controls()}
		<Slider label="Streckenlänge" bind:value={distanceM} min={FRESNEL_LIMITS.distanceM.min} max={FRESNEL_LIMITS.distanceM.max} scale="log" format={(v) => formatDistance(v, 1)} unitSymbol="m" />
		<Slider label="Frequenz" bind:value={frequencyHz} min={FRESNEL_LIMITS.frequencyHz.min} max={FRESNEL_LIMITS.frequencyHz.max} scale="log" format={(v) => formatFrequency(v, 2)} unitSymbol="Hz" />
		<Slider label="Antennenhöhe (beide)" bind:value={antennaHeightM} min={FRESNEL_LIMITS.antennaHeightM.min} max={FRESNEL_LIMITS.antennaHeightM.max} step={1} format={(v) => formatDistance(v, 0)} unitSymbol="m" />
		<Slider label="Hindernishöhe" bind:value={obstacleHeightM} min={FRESNEL_LIMITS.obstacleHeightM.min} max={FRESNEL_LIMITS.obstacleHeightM.max} step={1} format={(v) => formatDistance(v, 0)} unitSymbol="m" />
		<Slider label="Hindernisposition" bind:value={obstaclePosition} min={FRESNEL_LIMITS.obstaclePosition.min} max={FRESNEL_LIMITS.obstaclePosition.max} step={0.01} format={(v) => formatPercentage(v * 100, 0)} unitSymbol="% der Strecke" />
	{/snippet}

	{#snippet results()}
		<ResultCard label="r₁ am Hindernis" value={formatDistance(scene.clearance.radiusM, 2)} copyable={false} />
		<ResultCard label="Freier Anteil von r₁" value={formatPercentage(Math.max(0, scene.clearance.clearanceFraction) * 100, 0)} tone={statusTone} secondary={statusLabel} copyable={false} />
		<ResultCard label="Zusatzdämpfung (Knife-Edge)" value={formatNumber(scene.clearance.lossDb, 1)} unit="dB" tone={scene.clearance.lossDb > 0 ? 'warning' : 'success'} copyable={false} />
		{#if status !== 'frei'}
			<Callout tone="warning" title="60-%-Regel verletzt">
				Mindestens 60 % des Fresnel-Radius sollten frei bleiben, sonst wird die Verbindung durch Beugung merklich gedämpft. Antennen höher setzen oder eine höhere Frequenz (kleinere Zone) wählen.
			</Callout>
		{/if}
	{/snippet}
</WidgetFrame>
