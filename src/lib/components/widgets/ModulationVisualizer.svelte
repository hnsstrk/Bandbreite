<script lang="ts">
  ...
  const sampleCount = $derived(
    Math.min(MAX_SAMPLES, Math.max(MIN_SAMPLES, Math.round(SAMPLES_PER_CARRIER_CYCLE * carrierHz * durationS)))
  );
...
<div class="widget">
  ...
  <div class="readout">
    ...
    {/if}
    {#if overmodulated}
      <Badge tone="danger" icon="warning" srPrefix="Warnung">Übermodulation — die Hüllkurve klappt um</Badge>
    {/if}
    <Button
      size="sm"
      variant="ghost"
  ...
  <ChartFrame title="Zeitbereich" level={4} {description} minWidth={480}>
    <ModulationWaveforms {waveform} showEnvelope={kind === 'am'} {amDepth} modulatedLabel={activeKind.label} />
    {#snippet dataTable()}
      <table>
