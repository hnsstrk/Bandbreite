...
>
	<svg viewBox="0 0 {W} 240" aria-hidden="true">
		...
		<rect x={nextPulseX} y={AXIS_Y - PULSE_H} width={pulseDrawWidth} height={PULSE_H} fill="var(--color-series-1)" opacity="0.6" />
		<rect
			x={echoX}
	...
	{#snippet controls()}
		<Slider label="Zielentfernung R" bind:value={rangeM} min={RADAR_PULSE_LIMITS.rangeM.min} max={RADAR_PULSE_LIMITS.rangeM.max} scale="log" format={(v) => formatDistance(v, 1)} unitSymbol="m" />
		<Slider label="Impulsdauer τ" bind:value={pulseWidthS} min={RADAR_PULSE_LIMITS.pulseWidthS.min} max={RADAR_PULSE_LIMITS.pulseWidthS.max} scale="log" format={(v) => `${us(v)} µs`} unitSymbol="µs" />
		<Slider label="Pulswiederholfrequenz PRF" bind:value={prfHz} min={RADAR_PULSE_LIMITS.prfHz.min} max={RADAR_PULSE_LIMITS.prfHz.max} scale="log" format={(v) => `${formatNumber(v, 0)} Hz`} unitSymbol="Hz" />
	{/snippet}
	...
	{#snippet results()}
		...
		{#if result.ambiguous}
			<Callout tone="warning" title="Mehrdeutigkeit">
				Das Ziel liegt jenseits der eindeutigen Entfernung – das Echo wird dem falschen Impuls zugeordnet. PRF senken oder Entfernung verringern.
			</Callout>
