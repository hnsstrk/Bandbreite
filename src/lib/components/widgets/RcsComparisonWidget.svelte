<script lang="ts">
  ...
  const referenceOptions = RCS_OBJECTS.map((entry) => ({
    value: entry.id,
    label: `${entry.nameDE} (${formatRcs(entry.rcsM2)})`
  }));
...
>
      ...
      {#each RCS_OBJECTS as entry, i (entry.id)}
        ...
        >
          <rect
            ...
            width={W}
            height={ROW_H - 2}
            fill={active ? 'var(--color-brand-soft)' : 'transparent'}
            rx="4"
          />
          <text x={LABEL_W} y={y + 14} text-anchor="end" class="chart-axis-text" font-weight={active ? 600 : 400}
            >{entry.nameDE}</text
          >
          <rect
            x={BAR_X0}
            ...
            height={ROW_H - 12}
            rx="3"
            fill={entry.id === referenceId
              ? 'var(--color-series-3)'
              : active
                ? 'var(--color-series-1-solid)'
                : 'var(--color-series-1)'}
    ...
    <table>
      ...
      <tbody>
        {#each RCS_OBJECTS as entry (entry.id)}
          <tr
            ><td>{entry.nameDE}</td><td>{formatRcs(entry.rcsM2)}</td><td>{formatNumber(rcsToDbsm(entry.rcsM2), 1)}</td
            ></tr
          >
        {/each}
      </tbody>
  ...
  {#snippet results()}
    <ResultCard
      label="Gewählt: {selected.nameDE}"
      value={formatRcs(selected.rcsM2)}
      secondary="{formatNumber(rcsToDbsm(selected.rcsM2), 1)} dBsm"
      hint={selected.descriptionDE}
      copyable={false}
    />
    <ResultCard
