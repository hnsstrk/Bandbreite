<script lang="ts">
  ...
  const pulseW = (pulse: SsrPulse) =>
    pulse.kind === 'block' ? Math.min(pulse.widthUs * PX_PER_US, MAX_BLOCK_PX) : Math.max(3, pulse.widthUs * PX_PER_US);
...
<WidgetFrame
  ...
  stacked
  footnote="Beide Impulsgruppen sind maßstäblich in Mikrosekunden gezeichnet; der Zwischenraum steht sinnbildlich für die {formatNumber(
    timing.totalUs,
    1
...
>
  <svg viewBox="0 0 {W} {H}" aria-hidden="true">
    ...
    {#each txPulses as pulse (pulse.id)}
      ...
      />
      <text
        x={X0 + pulse.startUs * PX_PER_US + pulseW(pulse) / 2}
        y={TX_Y + 14}
        text-anchor="middle"
        class="pulse-label"
      >
    ...
    />
    <circle
      cx={txEndX + (rxX0 - txEndX) * travel}
      cy={TX_Y + (RX_Y - TX_Y) * travel}
      r="5"
  ...
  {#snippet results()}
    <ResultCard
      ...
      unit="µs"
      secondary="{formatNumber(timing.roundTripUs, 1)} µs Laufzeit + {formatNumber(
        TRANSPONDER_DELAY_US,
        0
      )} µs im Transponder"
      emphasis="hero"
