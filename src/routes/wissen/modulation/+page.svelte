...
<div class="article">
	<TableOfContents items={MODULATION_TOC} class="article__toc" />

	<div class="article__body">
		<LearningGoals goals={MODULATION_GOALS} />
		...
		<section aria-labelledby="warum-modulieren">
			...
			<Callout tone="tip" title="Merksatz">
				Modulation ist immer ein Handel: Bandbreite gegen Sendeleistung gegen Aufwand. Kein
				Verfahren gewinnt in allen drei Punkten.
			</Callout>
		</section>
		...
		<section aria-labelledby="analoge-verfahren">
			...
			<Card title="Modulations-Visualisierer" level={3} padding="md">
				<p class="card-intro">
					Verfahren umschalten, Regler bewegen und beobachten, was mit Zeitverlauf und Spektrum
					geschieht. Alle Kurven werden im Browser berechnet.
				</p>
				<ModulationVisualizer initialKind="am" />
			</Card>

			<SectionHeader
				title="Amplitudenmodulation, DSB und SSB"
				id="amplitudenmodulation"
				level={3}
			/>
			...
			<Callout tone="warning" title="Übermodulation">
				Ab einem Modulationsgrad über 100 % klappt die Hüllkurve um. Das Signal wird verzerrt und
				belegt weit mehr Bandbreite als zugewiesen — im Visualisierer gut zu sehen, wenn der
				Regler über 100 % steht.
			</Callout>

			<SectionHeader
				title="Frequenz- und Phasenmodulation"
				id="frequenz-und-phasenmodulation"
				level={3}
			/>
		...
		<section aria-labelledby="digitale-verfahren">
			...
			<Card title="Konstellation mit Rauschen" level={3} padding="md">
				<p class="card-intro">
					Der Regler verschlechtert den Störabstand. Bei welchem Wert berühren sich die Wolken
			...
			<SectionHeader
				title="Symbolrate, Bitrate, spektrale Effizienz"
				id="symbolrate-und-bitrate"
				level={3}
			/>
			...
			<Callout tone="info" title="Zusammenhang mit der Kanalkapazität">
				Wie viel spektrale Effizienz bei gegebenem Störabstand überhaupt möglich ist, beantwortet
				die Shannon-Hartley-Gleichung. Der
				<a href="/rechner/kanalkapazitaet/">Kanalkapazitäts-Rechner</a> zeigt die Grenze für eigene
				Werte.
			</Callout>
		</section>
		...
		<section aria-labelledby="mehrtraeger-und-spreizung">
			...
			<Callout tone="info" title="Beispiel GPS" source="IS-GPS-200">
				Der C/A-Code läuft mit 1,023 Mchip/s, die Navigationsdaten mit 50 Bit/s. Daraus folgt ein
				Prozessgewinn von rund 43 dB — der Grund, warum ein Satellitensignal weit unterhalb des
				Rauschpegels noch auswertbar ist.
			</Callout>
		</section>
				...
				<div class="table-scroll">
					<table>
						<caption class="sr-only">
							Modulationsverfahren mit Klasse, Bit je Symbol, spektraler Effizienz, typischer
							Bandbreite und Robustheit
						</caption>
						<thead>
							<tr>
		...
		<RelatedTopics href="/wissen/modulation/" />
	</div>
</div>
