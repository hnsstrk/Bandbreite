...
import { activePresetId, type PresetChip } from '$lib/components/calculators/presetChips.svelte';
import {
  FSPL_PARAMS,
  pickDistanceUnit,
  pickFrequencyUnit
...
import {
  ATTENUATION_SERIES,
  formatAttenuationValue
} from '$lib/components/charts/attenuationChartData';
...
describe('activePresetId', () => {
  ...
  it('Radarquerschnitt: PKW und Verkehrsflugzeug teilen sich 100 m²', () => {
    ...
    const active = RCS_PRESETS.filter(
      (preset) => activePresetId(RCS_PRESETS, 100, null) === preset.id
    );
    expect(active).toHaveLength(1);
  });
